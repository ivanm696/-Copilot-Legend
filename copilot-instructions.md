...
- Whenever possible, use in top-level scopes `export function x(вЂ¦) {вЂ¦}` instead of `export const x = (вЂ¦) => {вЂ¦}`. One advantage of using the `function` keyword is that the stack-trace shows a good name when debugging.
...
---
> Converted and distributed by [TomeVault](https://tomevault.io)
> This is a context snippet only. You'll also want the standalone SKILL.md file вЂ” [download at TomeVault](https://tomevault.io)
